                    ...
                    for e in estaciones:
                        try:
                            ...
                            if not (27 <= lat <= 44 and -19 <= lon <= 5):
                                continue
                            resultado.append({"id": e["indicativo"], "lat": lat, "lon": lon})
                        except Exception:
                            errores += 1
                            continue
...
async def _nombre_estacion(indicativo: str) -> str:
    """Obtiene el nombre legible de la estación AEMET."""
    try:
        # Buscar en el catálogo cargado
        for e in _aemet_estaciones:
            if e.get("id") == indicativo and "nombre" in e:
                return e["nombre"]
        ...
        async with httpx.AsyncClient(timeout=15) as client:
            ...
            if data_url:
                ...
                estaciones = r2.json()
                for e in estaciones:
                    if e.get("indicativo") == indicativo:
                        nombre = e.get("nombre", indicativo)
                        municipio = e.get("municipio", "")
                        provincia = e.get("provincia", "")
                        return f"{nombre} ({municipio}, {provincia})".strip(" (,)")
    except Exception as ex:
        logger.warning(f"No se pudo obtener nombre de estación {indicativo}: {ex}")
    return indicativo
...
async def clima_parcela(
    ...
    """
    Devuelve datos meteorológicos actuales de la estación AEMET más cercana.
    Internamente busca la estación, pero NO expone su nombre/código al frontend.
    """
    if not AEMET_API_KEY:
        raise HTTPException(status_code=503, detail="AEMET no configurado")

    indicativo = await _estacion_mas_cercana(lat, lon)
    if not indicativo:
        # Usar Madrid como último recurso
        indicativo = "5402"
        logger.warning("Sin estaciones AEMET, usando Madrid como fallback")
